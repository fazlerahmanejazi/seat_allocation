...
def get_seating_arrangement(rooms_capacity_data, courses_seats_data, days, half):
    ...
    for rooms in rooms_capacity_data:
        floor_wise_seats[str(rooms)[0]].append((int(rooms_capacity_data[rooms]), rooms))
        floor_wise_seats[str(rooms)[0]].append((rooms_capacity_data[rooms]-int(rooms_capacity_data[rooms]), rooms))
    ...
    tabular_arrangment = [[courses, sum(n for _, n in arrangement[courses]), len(arrangement[courses]), arrangement[courses]] for courses in arrangement]
    print(tabulate(tabular_arrangment, headers=['course_name', 'total_strength', 'room_count', 'arrangement'], tablefmt="psql"))
